...
## АЦП
Для работы с АЦП необходимо создать объект класса ```RPiPWM.Battery()```. Констркутор класса принимает на вход
два параметра:  
...
```RPiPWM.SSD1306_96_16()```, в зависимости от размеров дисплея. Входные параметры конструктора:
шина i2c, созданная при помощи модуля smbus (по умолчанию None, чтобы создать свой объект для общения с шиной),
а также адрес дисплея. При создании объекта не рекомендуется изменять значения по умолчанию 
(работоспособность в данном случае **не гарантируется**).  
